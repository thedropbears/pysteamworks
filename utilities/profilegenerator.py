...
def generate_trapezoidal_trajectory(
    ...
    triangular_max = math.sqrt(
            (2*abs(x)*a_pos*a_neg+a_neg*v_start**2-a_pos*v_final**2)/(a_neg-a_pos))
    ...
    t_decel = (abs(x)-t_cruise*(v_start + v_max)/2
    ...
    for i in range(0, num_segments+1):
        ...
        v = direction*((v_max-v_start)*i/num_segments+v_start)
        segments.append((
                direction*(((v+v_start)/2)*t_cruise*i/num_segments),
                direction*v, direction*a_pos))
    ...
    segments += [(direction
        * (x_cruise + v_max * (t_decel-t_cruise) * i / num_segments),
                  direction*v_max, 0) for i in range(1, num_segments+1)]
